namespace PC_Game_Pass_Notifier_Tests
{
	...
	public class DiscordApiManagerTests : IClassFixture<DiscordApiManagerTestFixture>
	{
		...
		{
			List<GamePassGame> gamesList = TestCaseUtilities.CreateGamePassGamesFromJsonFile("test_gameWithoutShortDescription.json");
			List<DiscordEmbed> embeds = new()
			{
				gamesList.First().ToDiscordEmbedWithIndex(1)
			};
			Assert.True(fixture.ApiManager.SendMessage(embeds, "Ein neues Spiel wurde soeben dem PC Game Pass hinzugefügt:"), "Failed to send message via Discord Webhook");
		}
	}
